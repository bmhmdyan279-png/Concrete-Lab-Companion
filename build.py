#!/usr/bin/env python3
"""
Concrete Lab Companion — Complete Build Script v1.0.0
=====================================================
...
def build_00_guide(wb):
    ...
    r += 1
    set_cell(ws, r, 1, "📝 اصلاحات نسبت به چاپ کتاب:", font=Font(name=FONT_FA, bold=True, size=11, color="C00000"), align=S["right_align"])
    r += 1
    set_cell(ws, r, 1, "۸ خطای فیزیکی/محاسباتی شناسایی و اصلاح شده — جزئیات در شیت ۲۵_خطاها", font=Font(name=FONT_FA, size=10), align=S["right_align"])
    ws.merge_cells(f"A{r}:K{r}")
...
def build_1_1_sieve(wb):
    ...
    set_cell(ws, r_check, 3, f'=IF($C$5=0,"",IF(ABS($C$5-SUM(C{start_row}:C{end_row-1}))/$C$5>0.003,"❌ اختلاف >0.3%","✅"))',
    ...
    set_cell(ws, r_fm, 3, f'=IF($C$5=0,"",ROUND(SUMPRODUCT((G{start_row}:G{end_row}=TRUE)*E{start_row}:E{end_row})/100,2))',
    ...
    for i, (size, (hi, lo)) in enumerate(isiri_limits.items()):
        ...
        set_cell(ws, row, 2, f'=IF(F{start_row+i}="","",F{start_row+i})', fill=S["calc_fill"], num_fmt='0.0')
...
def build_2_4_mortar(wb):
    ...
    set_cell(ws, r, 4, '=IF(OR(A6="",B6="",C6=""),"—",ROUND(AVERAGE(1.5*A6*9.80665*100/40^3,1.5*B6*9.80665*100/40^3,1.5*C6*9.80665*100/40^3),1))',
             fill=S["calc_fill"], num_fmt='0.0')
    ...
    ws.merge_cells(f"A{r}:C{r}")
    set_cell(ws, r, 4, f'=IF(COUNT({get_column_letter(1)}{r-2}:{get_column_letter(6)}{r-2})<6,"—",ROUND(AVERAGE({get_column_letter(1)}{r-2}*9.80665/1600,{get_column_letter(2)}{r-2}*9.80665/1600,{get_column_letter(3)}{r-2}*9.80665/1600,{get_column_letter(4)}{r-2}*9.80665/1600,{get_column_letter(5)}{r-2}*9.80665/1600,{get_column_letter(6)}{r-2}*9.80665/1600),1))',
             fill=S["calc_fill"], num_fmt='0.0')
...
def build_4_1_compressive(wb):
    ...
    set_cell(ws, r, 1, "مقاومت فشاری (MPa):", font=Font(name=FONT_FA, bold=True), align=S["right_align"])
    ws.merge_cells(f"A{r}:B{r}")
    set_cell(ws, r, 3, '=IF(OR(C7="",C11="—",C11=0),"—",ROUND(C7*1000/C11,1))',
...
def build_4_3_flexural(wb):
    ...
    set_cell(ws, r, 1, "مقاومت خمشی (MPa):", font=Font(name=FONT_FA, bold=True), align=S["right_align"])
    ws.merge_cells(f"A{r}:B{r}")
    set_cell(ws, r, 3, '=IF(OR(C5="",C6="",C7="",C8="",C11="خارج محدوده"),IF(C11="خارج محدوده","⚠️ تکرار","—"),IF(C10="مرکزی",ROUND(3*C8*C7/(2*C5*C6^2),2),ROUND(C8*C7/(C5*C6^2),2)))',
             fill=S["calc_fill"], num_fmt='0.00')
...
def build_4_5_schmidt(wb):
    ...
    ws.merge_cells(f"A{r}:B{r}")
    set_cell(ws, r, 3, f'=IF(OR(C{r-2}="",C{r-1}=0),"—",ROUND(SUMPRODUCT((ABS(A6:H7-C{r-2})<=6)*(A6:H7<>"")*A6:H7)/C{r-1},1))',
             fill=S["calc_fill"], num_fmt='0.0')
...
def build_23_dashboard(wb):
    ...
    r += 1
    set_cell(ws, r, 1, "پیشرفت:", font=Font(name=FONT_FA, bold=True), align=S["right_align"])
...
def build_24_qa(wb):
    ...
    tests = [
        ...
        ("T-004", "نمونه کتاب ۲-۴", "3340 kgf, A=1600", "≈20.5 MPa", '=IF(ABS(\'13_آزمایش_2-4\'!D15-20.5)<0.2,"✅ PASS","❌ FAIL")', "—"),
        ("T-005", "نمونه کتاب ۴-۱", "d=150, F=715 kN", "≈40.4 MPa", '=IF(ABS(\'17_آزمایش_4-1\'!C13-40.4)<0.2,"✅ PASS","❌ FAIL")', "—"),
    ]
    r += 1
...
def add_sieve_chart(wb):
    ...
    data_start = 18  # approximate row where chart data starts
    data_end = 25
...
def main():
    print("═" * 50)
    print("  Concrete Lab Companion — Build v1.0.0")
    print("═" * 50)

    wb = Workbook()

    print("\n🔨 Building sheets...")
    build_00_guide(wb); print("  ✅ 00_راهنما")
    build_01_info(wb); print("  ✅ 01_اطلاعات_آزمون")
    build_1_1_sieve(wb); print("  ✅ 02_آزمایش_1-1")
    build_1_2_moisture(wb); print("  ✅ 03_آزمایش_1-2")
    build_1_3_coarse_sg(wb); print("  ✅ 04_آزمایش_1-3")
    build_1_4_fine_sg(wb); print("  ✅ 05_آزمایش_1-4")
    build_1_5_unit_weight(wb); print("  ✅ 06_آزمایش_1-5")
    build_1_6_sand_equivalent(wb); print("  ✅ 07_آزمایش_1-6")
    build_1_7_shape(wb); print("  ✅ 08_آزمایش_1-7")
    build_1_8_absorption(wb); print("  ✅ 09_آزمایش_1-8")
    build_2_1_fresh_density(wb); print("  ✅ 10_آزمایش_2-1")
    build_2_2_vicat(wb); print("  ✅ 11_آزمایش_2-2")
    build_2_3_setting_time(wb); print("  ✅ 12_آزمایش_2-3")
    build_2_4_mortar(wb); print("  ✅ 13_آزمایش_2-4")
    build_3_1_slump(wb); print("  ✅ 14_آزمایش_3-1")
    build_3_2_bleeding(wb); print("  ✅ 15_آزمایش_3-2")
    build_3_3_concrete_unit_weight(wb); print("  ✅ 16_آزمایش_3-3")
    build_4_1_compressive(wb); print("  ✅ 17_آزمایش_4-1")
    build_4_2_tensile(wb); print("  ✅ 18_آزمایش_4-2")
    build_4_3_flexural(wb); print("  ✅ 19_آزمایش_4-3")
    build_4_4_upv(wb); print("  ✅ 20_آزمایش_4-4")
    build_4_5_schmidt(wb); print("  ✅ 21_آزمایش_4-5")
    build_22_report(wb); print("  ✅ 22_گزارش")
    build_23_dashboard(wb); print("  ✅ 23_داشبورد")
    build_24_qa(wb); print("  ✅ 24_QA_Test")
    build_25_errata(wb); print("  ✅ 25_خطاها_هشدارها")
    build_hidden_sheets(wb); print("  ✅ Hidden sheets (5)")
